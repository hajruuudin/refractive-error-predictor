"""
Random Forest Regression pipeline for predicting myopia_score, cvs_score, and astigmatism_score.

This script loads the composite target datasets generated by the SMOTE oversampling scripts, trains
a Random Forest Regressor for each target, evaluates performance using MAE, R², and 
cross-validated MAE, and saves feature importance and predictions to CSV files.

Note that the same setup of the Random Forest model is used for all three targets to allow for a 
fair comparison of results. The setup includes:
- n_estimators=100: A moderate number of trees to balance performance and training time.
- max_depth=10: Limits tree depth to prevent overfitting on the training data.
- min_samples_leaf=5: Requires at least 5 samples in a leaf node to further reduce overfitting.
- max_samples=0.8: Each tree is trained on a random 80% subset of the data to increase diversity among trees.
- max_features='sqrt': Each tree considers a random subset of features (square root of total) when looking for the best split, which also helps to reduce overfitting and improve generalization.
