"""
PART 02B: XGBOOST MODEL TRAINING

This script trains XGBoost regression models for each of the three composite targets (Myopia Score, 
...
3. Trains an XGBoost Regressor on the training data
...
class XGBoostTrainer:
    """Train and evaluate XGBoost models for refractive error prediction"""

    def __init__(self, myopia_path, cvs_path, astigmatism_path):
        """
        Initialize trainer with the three SMOTE-balanced CSV files.

        Args:
            myopia_path:      Path to training_data_smote_myopia.csv
            cvs_path:         Path to training_data_smote_computervs.csv
            astigmatism_path: Path to training_data_smote_astigmatism.csv
        """
        self.datasets = {
    ...
    def train_target(self, target, test_size=0.2):
        """
        Train an XGBoost model for a single composite target.

        Args:
            target:    Target column name (must match column in its CSV)
    ...
    def train_all(self):
        """Train XGBoost models for all three composite targets"""
        print(f"\n{'='*70}")
...
def main():
    ...
    # Input CSV paths (one per composite target)
    ...
    # Validate all three files exist before doing anything
    for path in [myopia_path, cvs_path, astigmatism_path]:
        if not os.path.exists(path):
            print(f"Error: {path} not found. Run the SMOTE script first.")
            return

    output_dir = os.path.join(script_dir, 'model_results_xgb')

    print("=" * 70)
    print("XGBOOST PIPELINE - MODEL TRAINING")
    print("=" * 70)

    # Initialize and train
    trainer = XGBoostTrainer(myopia_path, cvs_path, astigmatism_path)
    trainer.train_all()
