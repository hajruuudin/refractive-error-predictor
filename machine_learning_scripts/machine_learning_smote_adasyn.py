"""
PART 01F: ADASYN for Machine Learning Pipeline

ADASYN (Adaptive Synthetic Sampling) variant that adaptively generates
synthetic samples based on density and difficulty of learning. Rather than
generating the same number of samples for each minority class sample,
ADASYN generates MORE samples for harder-to-learn examples (those with
fewer neighbors of the same class) and FEWER samples for easier examples.

Benefits:
- Adaptive: focuses on difficult-to-learn regions
- Better for imbalanced datasets
- Generates varying amounts of samples per minority point
- No clustering required
- Excellent for small datasets with varying density regions
...
4. Apply ADASYN to balance classes
...
def apply_adasyn_for_ml(df, primary_target):
    input_cols = [
        'age', 'gender', 'daily_screen_time', 'continuous_usage', 'intensity',
        'lighting', 'multi_device', 'phone_distance', 'monitor_distance',
        'blue_light_filter', 'before_bed_usage', 'profession', 'outdoor_activity',
    ...
    # Remove classes with fewer than 2 samples (ADASYN needs at least 2 per class)
    ...
    # ADASYN is strict about n_neighbors; cap at 2 for very small classes
    n_neighbors = min(2, max(1, min_class_size - 1))
    print(f"  Using n_neighbors={n_neighbors} for ADASYN")

    # Try ADASYN first; fall back to vanilla SMOTE if it fails
    try:
        adasyn = ADASYN(sampling_strategy='auto', random_state=42, n_neighbors=n_neighbors)
        X_resampled, y_resampled = adasyn.fit_resample(X, y)
        print(f"  Successfully applied ADASYN")
    except ValueError as e:
        print(f"  ADASYN failed ({str(e)}), falling back to vanilla SMOTE...")
    ...
    print(f"  Total samples after ADASYN: {len(X_resampled)}")
...
def main():
    ...
    print("\nApplying ADASYN with Weighted Average targets...")
    myopia_balanced_df = apply_adasyn_for_ml(df, primary_target="myopia_score")
    ...
    cvs_balanced_df = apply_adasyn_for_ml(df, primary_target="cvs_score")
    ...
    astigmatism_balanced_df = apply_adasyn_for_ml(df, primary_target="astigmatism_score")
    ...
    print("\nAll ADASYN training data generated successfully!")
